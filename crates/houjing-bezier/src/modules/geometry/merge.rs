...
pub fn merge_curves_sequentially(mut curves: Vec<Vec<Vec2>>) -> Vec<Vec<Vec2>> {
    if curves.len() < 2 {
        return curves;
    }
...
mod tests {
    ...
    fn test_merge_split_linear_curves() {
        let linear_curve_1 = vec![Vec2::new(0.0, 0.0), Vec2::new(3.0, 4.0)];
        let linear_curve_2 = vec![Vec2::new(3.0, 4.0), Vec2::new(6.0, 8.0)];
        let linear_curve_3 = vec![Vec2::new(3.0, 4.0), Vec2::new(8.0, 8.0)];
        let original = vec![Vec2::new(0.0, 0.0), Vec2::new(6.0, 8.0)];
        ...
        let vertical_curve_1 = vec![Vec2::new(10.0, 0.0), Vec2::new(10.0, 10.0)];
        let vertical_curve_2 = vec![Vec2::new(10.0, 10.0), Vec2::new(10.0, 20.0)];
        assert!(merge_split_bezier_curves(&vertical_curve_1, &vertical_curve_2).is_some());

        // merge horizontal curve
        let horizontal_curve_1 = vec![Vec2::new(10.0, 0.0), Vec2::new(20.0, 0.0)];
        let horizontal_curve_2 = vec![Vec2::new(20.0, 0.0), Vec2::new(30.0, 0.0)];
    ...
    fn test_merge_split_cubic_curves_captured_from_user() {
        ...
        let left = vec![
            Vec2::new(-245.99219, -150.95313),
            Vec2::new(-157.95697, -25.547081),
            Vec2::new(-22.974106, 5.259905),
            Vec2::new(78.418434, 52.253418),
        ];

        let right = vec![
            Vec2::new(78.418434, 52.253418),
            Vec2::new(176.16669, 97.55787),
            Vec2::new(242.69585, 157.90619),
            Vec2::new(205.84375, 332.5625),
        ];
    ...
    fn test_merge_curves_sequentially() {
        ...
        let curve2 = vec![Vec2::new(1.0, 1.0), Vec2::new(2.0, 2.0)];
        let curve3 = vec![Vec2::new(2.0, 2.0), Vec2::new(3.0, 3.0)];
        ...
        assert_eq!(result[0][0], Vec2::new(0.0, 0.0));
        assert_eq!(result[0][1], Vec2::new(3.0, 3.0));

        // Test case 2: Mix of mergeable and non-mergeable curves
        let curve_a = vec![Vec2::new(0.0, 0.0), Vec2::new(1.0, 0.0)]; // A→B
        let curve_b = vec![Vec2::new(1.0, 0.0), Vec2::new(2.0, 0.0)]; // B→C (can merge with A)
        let curve_c = vec![Vec2::new(5.0, 5.0), Vec2::new(6.0, 5.0)]; // Disconnected
        let curve_d = vec![Vec2::new(6.0, 5.0), Vec2::new(7.0, 5.0)]; // D→E (can merge with C)
        ...
        let curve_x = vec![Vec2::new(0.0, 0.0), Vec2::new(1.0, 0.0)];
        let curve_y = vec![Vec2::new(5.0, 5.0), Vec2::new(6.0, 5.0)]; // Disconnected
        ...
        let single_curve = vec![Vec2::new(0.0, 0.0), Vec2::new(1.0, 1.0)];
